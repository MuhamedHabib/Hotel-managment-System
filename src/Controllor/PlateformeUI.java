...
public class PlateformeUI implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        List<offre> lss=new ArrayList<>();
        ServiceOffre s=new ServiceOffre();
        ServiceOffre ss=new ServiceOffre();
        try {
            ls= s.read();
            lss=ss.read();
            try {
                for (int i = 0; i < ls.size(); i++) {
                    ...
                    ItemControllor item2Controllor = fxmlLoader.getController();
                    item2Controllor.setData(ls.get(i));
                ...
                for(int j=0;j<lss.size();j++){
                    ...
                    HBox cardBox = fxmlLoader.load();
                    item2controllor item2Controllor = fxmlLoader.getController();
                    item2Controllor.setData(lss.get(j));
